...
indie_header = """# This .ifd was generated by a limited-commercial version
# of Houdini and should not be used to generate images
# for purposes outside those allowed by the limited-commercial
# agreement.  Furthermore, the following line should not
# be deleted.  Thank you for your cooperation.
# limited_commercial_ifd
"""


def file_cmp_clean(file_test, file_control):
    with open(file_test, "r") as a:
        a_data = a.read()

    tag = "limited_commercial_ifd\n"
    try:
        offset = a_data.index(tag)
        clean_data = a_data[offset + len(tag) :]
    except ValueError:
        clean_data = a_data

    with open(file_test, "w") as a:
        a.write(clean_data)

    return filecmp.cmp(file_test, file_control)
...
class TestInstance(TestRoot):
    ...
    def test_fast_instance_mb(self):
        line = self.instance.createNode("line")
        line.parm("points").set(3)
        line.parm("type").set("points")
        line.parm("originx").setExpression("$FF")
        line.setRenderFlag(True)
        self.instance.parm("instancepath").set(self.geo1.path())
        self.instance.parm("ptinstance").set("fast")
...
class TestShapes(TestRoot):
    ...
    def test_plymesh_faceIndices(self):
        ptg = self.geo.parmTemplateGroup()
        parm = hou.properties.parmTemplate("pbrt-v4", "pbrt_computeN")
        ptg.append(parm)
        self.geo.setParmTemplateGroup(ptg)
        ...
        self.rop.parm("pbrt_allow_geofiles").set("always")
        self.compare_scene()
        self.compare_ply("+attribwrangle1+0")
    ...
    def test_points(self):
        ptg = self.geo.parmTemplateGroup()
        parm = hou.properties.parmTemplate("pbrt-v4", "pbrt_renderpoints")
        ptg.append(parm)
        self.geo.setParmTemplateGroup(ptg)
        self.geo.parm("pbrt_renderpoints").set(True)
        line = self.geo.createNode("line")
        line.parm("points").set(5)
        line.parm("type").set("points")
        self.compare_scene()

    def test_points_ptmotion(self):
        ptg = self.geo.parmTemplateGroup()
        parm = hou.properties.parmTemplate("pbrt-v4", "pbrt_renderpoints")
        ptg.append(parm)
        self.geo.setParmTemplateGroup(ptg)
    ...
    def test_points_override(self):
        ...
        material.parm("local1_name1").set("reflectance")
        material.parm("local1_type1").set("color")
        material.parm("group1").set("0-2")
        material.parmTuple("local1_cval1").set([0.9, 0, 0])
        material.setFirstInput(line)
        material.setRenderFlag(True)
        self.compare_scene()
    ...
    def test_volume_sigma_many_mapped(self):
        sigma_a_0 = build_volume(self.geo, rgb=True, name="sigma_a")
        sigma_s_0 = build_volume(self.geo, rgb=True, name="sigma_s")
        merge_0 = self.geo.createNode("merge")
        merge_0.setInput(0, sigma_a_0)
        merge_0.setInput(1, sigma_s_0)
        wrangler_0 = self.geo.createNode("attribwrangle")
        wrangler_0.parm("class").set("primitive")
        wrangler_0.parm("snippet").set("i@medium_grids = 0;")
        wrangler_0.setFirstInput(merge_0)

        sigma_a_1 = build_volume(self.geo, rgb=True, name="sigma_a")
        sigma_s_1 = build_volume(self.geo, rgb=True, name="sigma_s")
        merge_1 = self.geo.createNode("merge")
        merge_1.setInput(0, sigma_a_1)
        merge_1.setInput(1, sigma_s_1)
        wrangler_1 = self.geo.createNode("attribwrangle")
        wrangler_1.parm("class").set("primitive")
        wrangler_1.parm("snippet").set("i@medium_grids = 1;")
        wrangler_1.setFirstInput(merge_1)

        merge = self.geo.createNode("merge")
        merge.setInput(0, wrangler_0)
        merge.setInput(1, wrangler_1)
        merge.setRenderFlag(True)
        self.compare_scene()
