...
def file_cmp_as_str(file_a, file_b):
    with open(file_a, "r") as a:
        a_data = a.read()
    if a_data.startswith(indie_header[:20]):
        a_data = a_data[len(indie_header):]
...
class TestROP(TestRoot):
    ...
    def compare_scene(self):
        self.rop.render()
        self.assertTrue(file_cmp_as_str(self.testfile, self.basefile))
